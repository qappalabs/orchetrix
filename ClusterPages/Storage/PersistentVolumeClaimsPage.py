...
class PersistentVolumeClaimsHeader(QHeaderView):
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace), 3 (Storage class),  4 (Size), 5 (Pods), 6 (Age), 7 (Status)
...
class PersistentVolumeClaimsPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_persistent_volume_claims = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + Storage class + Size + Pods + Age + Status + [Action]
    ...
    def load_data(self):
        ...
        for row, ingress_classes in enumerate(self.ingress_classes_data):
            ...
            storage_class = ingress_classes[2]
            size = ingress_classes[3]
            pods = ingress_classes[4]
            age_str = ingress_classes[5]
            ...
            self.table.setItem(row, 5, item_pods)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, perstent_volume_claims):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, perstent_volume_claims))
        return checkbox

    def create_checkbox_container(self, row, perstent_volume_claims):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, perstent_volume_claims)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, perstent_volume_claims):
        if state == Qt.CheckState.Checked.value:
            self.selected_persistent_volume_claims.add(perstent_volume_claims)
        else:
            self.selected_persistent_volume_claims.discard(perstent_volume_claims)
        ...
        print("Selected perstent_volume_claims:", self.selected_persistent_volume_claims)
    ...
    def handle_action(self, action, row):
        perstent_volume_claims = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing perstent_volume_claims: {perstent_volume_claims}")
        elif action == "Delete":
            print(f"Deleting perstent_volume_claims: {perstent_volume_claims}")

    #------- Row Selection -------
    def select_perstent_volume_claim(self, row):
        self.table.selectRow(row)
        print(f"Selected perstent_volume_claims: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_perstent_volume_claim(row)