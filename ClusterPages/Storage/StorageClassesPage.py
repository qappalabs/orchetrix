...
class StorageClassesHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Provisioner), 3 (Reclaim Policy ), 5 (Age)
        self.sortable_columns = {1, 2, 3, 5}
...
class StorageClassesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_storage = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Provisioner + Reclaim Policy + Default + Age +  [Action]
        ...
        custom_header = StorageClassesHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 2: Provisioner (fixed)
        self.table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(2, 150)

        # Column 3: Reclaim Policy (fixed)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)

        # Column 4: Default (fixed)
    ...
    def load_data(self):
        ...
        for row, ingresses in enumerate(self.ingresses_data):
            ...
            name = ingresses[0]
            provisioner = ingresses[1]
            reclaim_policy = ingresses[2]
            default = ingresses[3]
            age_str = ingresses[4]
            ...
            # Provisioner -> column 2
            item_provisioner = QTableWidgetItem(provisioner)
            item_provisioner.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_provisioner.setForeground(QColor("#e2e8f0"))
            item_provisioner.setFlags(item_provisioner.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 2, item_provisioner)

            # Reclaim Policy -> column 3
            item_reclaim_policy = QTableWidgetItem(reclaim_policy)
            item_reclaim_policy.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_reclaim_policy.setForeground(QColor("#e2e8f0"))
            item_reclaim_policy.setFlags(item_reclaim_policy.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_reclaim_policy)

            # Default -> column 3
            item_default = QTableWidgetItem(default)
            item_default.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_default.setForeground(QColor("#e2e8f0"))
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, storage_classes_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, storage_classes_name))
        return checkbox

    def create_checkbox_container(self, row, storage_classes_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, storage_classes_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, storage_classes_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_storage.add(storage_classes_name)
        else:
            self.selected_storage.discard(storage_classes_name)
        ...
        print("Selected storage classes:", self.selected_storage)
    ...
    def handle_action(self, action, row):
        storage_classes_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing storage classes: {storage_classes_name}")
        elif action == "Delete":
            print(f"Deleting storage classes: {storage_classes_name}")

    #------- Row Selection -------
    def select_storage_classes(self, row):
        self.table.selectRow(row)
        print(f"Selected storage classes: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_storage_classes(row)