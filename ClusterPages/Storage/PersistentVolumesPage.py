...
class PersistentVolumeHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Storage class), 3 (Capicity),  4 (Claim), 5 (Age), 6 (Status)
...
class PersistentVolumesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_persistent_volume = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        title = QLabel("Persistent Volumes")
        title.setStyleSheet("""
            font-size: 20px;
            font-weight: bold;
            color: #ffffff;
        ...
        # Table: 8 columns total: [Checkbox] + Name + Storage class + Capacity + Claim + Status + Age +  [Action]
        ...
        custom_header = PersistentVolumeHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 2: Storage class (fixed)
        self.table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(2, 150)

        # Column 3: Capacity (fixed)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)

        # Column 4: Claim (fixed)
    ...
    def load_data(self):
        ...
        for row, ingress_classes in enumerate(self.ingress_classes_data):
            ...
            storage_class = ingress_classes[1]
            capacity = ingress_classes[2]
            claim = ingress_classes[3]
            age_str = ingress_classes[4]
            status = ingress_classes[5]
            ...
            # Storage class -> column 2
            item_storage_class = QTableWidgetItem(storage_class)
            item_storage_class.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_storage_class.setForeground(QColor("#e2e8f0"))
            item_storage_class.setFlags(item_storage_class.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 2, item_storage_class)

            # Capacity -> column 3
            item_capacity = QTableWidgetItem(capacity)
            item_capacity.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_capacity.setForeground(QColor("#e2e8f0"))
            item_capacity.setFlags(item_capacity.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_capacity)

            # Claim -> column 4
            item_claim = QTableWidgetItem(claim)
            item_claim.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_claim.setForeground(QColor("#e2e8f0"))
            item_claim.setFlags(item_claim.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_claim)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, persistent_volume_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, persistent_volume_name))
        return checkbox

    def create_checkbox_container(self, row, persistent_volume_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, persistent_volume_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, persistent_volume_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_persistent_volume.add(persistent_volume_name)
        else:
            self.selected_persistent_volume.discard(persistent_volume_name)
        ...
        print("Selected persistent volumes:", self.selected_persistent_volume)
    ...
    def handle_action(self, action, row):
        persistent_volume_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing persistent volumes: {persistent_volume_name}")
        elif action == "Delete":
            print(f"Deleting persistent volumes: {persistent_volume_name}")

    #------- Row Selection -------
    def select_persistent_volume(self, row):
        self.table.selectRow(row)
        print(f"Selected persistent volumes: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_persistent_volume(row)