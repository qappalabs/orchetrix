...
class DefinitionsHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Resource), 2 (Group), 3 (Version),  4 (Scope), 5 (Age)
...
class DefinitionsPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.selected_difinitions = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Resource + Group + Version + Scope +  Age +  [Action]
        ...
        custom_header = DefinitionsHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 2: Group (fixed)
        self.table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(2, 150)

        # Column 3: Version (fixed)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)

        # Column 4: Scope (fixed)
    ...
    def load_data(self):
        ...
        self.ingress_classes_data = [
            ["docker.io-hostpath", "kube-system", "<none>", "<none>", "<none>"],
        ]
        self.table.setRowCount(len(self.ingress_classes_data))

        for row, ingress_classes in enumerate(self.ingress_classes_data):
            self.table.setRowHeight(row, 40)
            resource = ingress_classes[0]
            group = ingress_classes[1]
            version = ingress_classes[2]
            scope = ingress_classes[3]
            age_str = ingress_classes[4]
    

            # Create checkbox in column 0
            checkbox_container = self.create_checkbox_container(row, resource)
            ...
            try:
                ...
                pass
            # Resource -> column 1
            item_resource = QTableWidgetItem(resource)
            item_resource.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
            item_resource.setForeground(QColor("#e2e8f0"))
            item_resource.setFlags(item_resource.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 1, item_resource)

            # Group -> column 2
            item_group = QTableWidgetItem(group)
            item_group.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_group.setForeground(QColor("#e2e8f0"))
            ...
            self.table.setItem(row, 3, item_version)
            ...
            self.table.setItem(row, 4, item_scope)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, difinitions_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, difinitions_name))
        return checkbox

    def create_checkbox_container(self, row, difinitions_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, difinitions_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, difinitions_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_difinitions.add(difinitions_name)
        else:
            self.selected_difinitions.discard(difinitions_name)
        ...
        print("Selected difinitions:", self.selected_difinitions)
    ...
    def handle_action(self, action, row):
        difinitions_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing difinitions: {difinitions_name}")
        elif action == "Delete":
            print(f"Deleting difinitions: {difinitions_name}")

    #------- Row Selection -------
    def select_difinitions(self, row):
        self.table.selectRow(row)
        print(f"Selected difinitions: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_difinitions(row)