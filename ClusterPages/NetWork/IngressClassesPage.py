...
class IngressClassesHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace), 3 (Controller),  4 (API Group), 5 (Scope), 6 (Kind)
...
class IngressClassesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_ingress_classes = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        custom_header = IngressClassesHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 3: Controller (fixed)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)

        # Column 4: API Group (fixed)
        self.table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)
       
        # Column 5: Scope (fixed)
    ...
    def load_data(self):
        ...
        for row, ingress_classes in enumerate(self.ingress_classes_data):
            ...
            controller = ingress_classes[2]
            api_group = ingress_classes[3]
            scope = ingress_classes[4]
            kind = ingress_classes[5]
            ...
            # Controllers -> column 3
            item_controller = QTableWidgetItem(controller)
            item_controller.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_controller.setForeground(QColor("#e2e8f0"))
            item_controller.setFlags(item_controller.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_controller)

            # API Group -> column 4
            item_apigroup = QTableWidgetItem(api_group)
            item_apigroup.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_apigroup.setForeground(QColor("#e2e8f0"))
            item_apigroup.setFlags(item_apigroup.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_apigroup)

            # Scope -> column 5
            item_scope = QTableWidgetItem(scope)
            item_scope.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_scope.setForeground(QColor("#e2e8f0"))
            item_scope.setFlags(item_scope.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 5, item_scope)
            ...
            self.table.setItem(row, 6, item_kind)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, ingress_classes_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, ingress_classes_name))
        return checkbox

    def create_checkbox_container(self, row, ingress_classes_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, ingress_classes_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, ingress_classes_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_ingress_classes.add(ingress_classes_name)
        else:
            self.selected_ingress_classes.discard(ingress_classes_name)
        ...
        print("Selected ingress_classes:", self.selected_ingress_classes)
    ...
    def handle_action(self, action, row):
        ingress_classes_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing ingress_classes: {ingress_classes_name}")
        elif action == "Delete":
            print(f"Deleting ingress_classes: {ingress_classes_name}")

    #------- Row Selection -------
    def select_ingress_classes(self, row):
        self.table.selectRow(row)
        print(f"Selected ingress_classes: {self.table.item(row, 1).text()}")
