...
class NretworkPoliciesHeader(QHeaderView):
    """
...
class NetworkPoliciesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_networkpolicy = set()  # Track which deployments are checked
        self.setup_ui()
        self.load_data()
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + PolicyType + Age +  [Action]
        ...
        custom_header = NretworkPoliciesHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def load_data(self):
        ...
        for row, ingresses in enumerate(self.ingresses_data):
            ...
            item_policytype = QTableWidgetItem(policy_type)
            item_policytype.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_policytype.setForeground(QColor("#e2e8f0"))
            item_policytype.setFlags(item_policytype.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_policytype)
            # Age -> column 4
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, networkpolicy_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, networkpolicy_name))
        return checkbox

    def create_checkbox_container(self, row, networkpolicy_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, networkpolicy_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, networkpolicy_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_networkpolicy.add(networkpolicy_name)
        else:
            self.selected_networkpolicy.discard(networkpolicy_name)
        print("Selected networkpolicy:", self.selected_networkpolicy)
    ...
    def handle_action(self, action, row):
        networkpolicy_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing networkpolicy: {networkpolicy_name}")
        elif action == "Delete":
            print(f"Deleting networkpolicy: {networkpolicy_name}")

    #------- Row Selection -------
    def select_networkpolicy(self, row):
        self.table.selectRow(row)
        print(f"Selected networkpolicy: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_networkpolicy(row)