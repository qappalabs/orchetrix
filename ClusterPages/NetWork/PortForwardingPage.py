...
class PortForwardingHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace), 3 (Kind),  4 (Pod Port), 5 (Local Port), 6 (Protocol), 7 (Status)
        self.sortable_columns = {1, 2, 3, 4, 5, 6,7}
...
class PortForwardingPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_port_forwarding = set()  # Track which deployments are checked
        self.setup_ui()
        self.load_data()
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + LoadBalancers + Rule + Age +  [Action]
        self.table = QTableWidget()
        self.table.setColumnCount(9)
        headers = ["", "Name", "Namespace", "Kind", "Pod Port", "Lock Port", "Protocol", "Status",  "⋮"]
        self.table.setHorizontalHeaderLabels(headers)
        
        # Use the custom header for selective header-based sorting.
        custom_header = PortForwardingHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 4: Pod Port(fixed)
        self.table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)
       
        # Column 5: Local Port (fixed)
        self.table.horizontalHeader().setSectionResizeMode(5, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(5, 80)
       
       # Column 6: Protocol (fixed)
    ...
    def load_data(self):
        ...
        for row, ingress_classes in enumerate(self.ingress_classes_data):
            ...
            kind = ingress_classes[2]
            pod_port = ingress_classes[3]
            local_port = ingress_classes[4]
            protocol = ingress_classes[5]
            ...
            item_kind.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_kind.setForeground(QColor("#e2e8f0"))
            item_kind.setFlags(item_kind.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_kind)

            # Pod Ports -> column 4
            item_pod_ports = QTableWidgetItem(pod_port)
            item_pod_ports.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_pod_ports.setForeground(QColor("#e2e8f0"))
            item_pod_ports.setFlags(item_pod_ports.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_pod_ports)

            # local port -> column 5
            item_local_ports = QTableWidgetItem(local_port)
            item_local_ports.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_local_ports.setForeground(QColor("#e2e8f0"))
            item_local_ports.setFlags(item_local_ports.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 5, item_local_ports)

            # Protocol -> column 6
            item_protocol = QTableWidgetItem(protocol)
            item_protocol.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_protocol.setForeground(QColor("#e2e8f0"))
            item_protocol.setFlags(item_protocol.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 6, item_protocol)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, port_forwarding_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, port_forwarding_name))
        return checkbox

    def create_checkbox_container(self, row, port_forwarding_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, port_forwarding_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, port_forwarding_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_port_forwarding.add(port_forwarding_name)
        else:
            self.selected_port_forwarding.discard(port_forwarding_name)
        print("Selected port_frowarding:", self.selected_port_forwarding)
    ...
    def handle_action(self, action, row):
        port_forwarding_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing port_forwarding: {port_forwarding_name}")
        elif action == "Delete":
            print(f"Deleting port_forwarding: {port_forwarding_name}")

    #------- Row Selection -------
    def select_port_forwarding(self, row):
        self.table.selectRow(row)
        print(f"Selected port_forwarding: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_port_forwarding(row)