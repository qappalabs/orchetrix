...
class IngressesHeader(QHeaderView):
    """
...
class IngressesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_ingresses = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        custom_header = IngressesHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def load_data(self):
        ...
        for row, ingresses in enumerate(self.ingresses_data):
            ...
            # LoadBlancers -> column 3
            item_loadblancers = QTableWidgetItem(loadblancers)
            item_loadblancers.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_loadblancers.setForeground(QColor("#e2e8f0"))
            item_loadblancers.setFlags(item_loadblancers.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_loadblancers)

            # Rule -> column 3
            item_rule = QTableWidgetItem(rule)
            item_rule.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_rule.setForeground(QColor("#e2e8f0"))
            item_rule.setFlags(item_rule.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_rule)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, ingresses_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, ingresses_name))
        return checkbox

    def create_checkbox_container(self, row, ingresses_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, ingresses_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, ingresses_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_ingresses.add(ingresses_name)
        else:
            self.selected_ingresses.discard(ingresses_name)
        ...
        print("Selected ingresses:", self.selected_ingresses)
    ...
    def handle_action(self, action, row):
        ingresses_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing ingresses: {ingresses_name}")
        elif action == "Delete":
            print(f"Deleting ingresses: {ingresses_name}")

    #------- Row Selection -------
    def select_ingresses(self, row):
        self.table.selectRow(row)
        print(f"Selected ingresses: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_ingresses(row)