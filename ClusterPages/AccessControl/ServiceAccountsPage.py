...
class ServiceAccountsHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace),  3 (Age)
...
class ServiceAccountsPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_service_account = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + Age +  [Action]
        ...
        custom_header = ServiceAccountsHeader(Qt.Orientation.Horizontal, self.table)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, service_accounts_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, service_accounts_name))
        return checkbox

    def create_checkbox_container(self, row, service_accounts_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, service_accounts_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, service_accounts_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_service_account.add(service_accounts_name)
        else:
            self.selected_service_account.discard(service_accounts_name)
        ...
        print("Selected Service Account:", self.selected_service_account)
    ...
    def handle_action(self, action, row):
        service_accounts_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing Service Account: {service_accounts_name}")
        elif action == "Delete":
            print(f"Deleting Service Account: {service_accounts_name}")

    #------- Row Selection -------
    def select_service_account(self, row):
        self.table.selectRow(row)
        print(f"Selected Service Account: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_service_account(row)