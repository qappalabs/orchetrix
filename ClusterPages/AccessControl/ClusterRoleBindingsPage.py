...
class ClusterRoleBindingsHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Bindings),  3 (Age)
        self.sortable_columns = {1, 2, 3}
...
class ClusterRoleBindingsPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_cluster_role_bindings = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        title = QLabel("Cluster Role Bindings")
        title.setStyleSheet("""
            font-size: 20px;
            font-weight: bold;
            color: #ffffff;
        ...
        # Table: 8 columns total: [Checkbox] + Name + Bindings + Age +  [Action]
        self.table = QTableWidget()
        self.table.setColumnCount(5)
        headers = ["", "Name", "Bindings", "Age",  "⋮"]
        self.table.setHorizontalHeaderLabels(headers)
        
        # Use the custom header for selective header-based sorting.
        custom_header = ClusterRoleBindingsHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def load_data(self):
        ...
        Example data:
          ...
          Bindings: kube-system
          Age: 70d
        """
        self.ingresses_data = [
            ["docker.io-hostpath", "kube-system", "70d",]
        ]
        ...
        for row, ingresses in enumerate(self.ingresses_data):
            ...
            # Bindings -> column 2
            item_bibding = QTableWidgetItem(bindings)
            item_bibding.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_bibding.setForeground(QColor("#e2e8f0"))
            item_bibding.setFlags(item_bibding.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 2, item_bibding)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, cluster_role_bindings_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, cluster_role_bindings_name))
        return checkbox

    def create_checkbox_container(self, row, cluster_role_bindings_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, cluster_role_bindings_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, cluster_role_bindings_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_cluster_role_binding.add(cluster_role_bindings_name)
        else:
            self.selected_cluster_role_binding.discard(cluster_role_bindings_name)
        ...
        print("Selected CLuster Role Bindings:", self.selected_cluster_role_bindings)
    ...
    def handle_action(self, action, row):
        cluster_role_bindings_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing Role: {cluster_role_bindings_name}")
        elif action == "Delete":
            print(f"Deleting Role: {cluster_role_bindings_name}")

    #------- Row Selection -------
    def select_cluster_role_bindings(self, row):
        self.table.selectRow(row)
        print(f"Selected Cluster Role Bindings: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_cluster_role_bindings(row)