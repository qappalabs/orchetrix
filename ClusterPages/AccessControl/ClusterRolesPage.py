...
class ClusterRolesHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name),  2 (Age)
        self.sortable_columns = {1, 2}
...
class ClusterRolesPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_cluster_roles = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        custom_header = ClusterRolesHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def custom_table_mousePressEvent(self, event):
        ...
        if index.isValid():
            row = index.row()
            if index.column() != 4:  # Skip action column
    ...
    def load_data(self):
        ...
        for row, ingresses in enumerate(self.ingresses_data):
            ...
            self.table.setItem(row, 2, item_age)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, cluster_role_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, cluster_role_name))
        return checkbox

    def create_checkbox_container(self, row, cluster_role_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, cluster_role_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, cluster_role_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_cluster_roles.add(cluster_role_name)
        else:
            self.selected_cluster_roles.discard(cluster_role_name)
        ...
        print("Selected Cluster Roles:", self.selected_cluster_roles)
    ...
    def handle_action(self, action, row):
        cluster_role_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing Cluster Roles: {cluster_role_name}")
        elif action == "Delete":
            print(f"Deleting Cluster Roles: {cluster_role_name}")

    #------- Row Selection -------
    def select_cluster_role(self, row):
        self.table.selectRow(row)
        print(f"Selected Cluster Roles: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_cluster_role(row)