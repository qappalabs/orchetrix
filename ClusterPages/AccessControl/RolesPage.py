...
class RolesPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.selected_role = set()  # Track which deployments are checked
    ...
    def setup_ui(self):
        ...
        custom_header = RolesHeader(Qt.Orientation.Horizontal, self.table)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, role_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, role_name))
        return checkbox

    def create_checkbox_container(self, row, role_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, role_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, role_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_role.add(role_name)
        else:
            self.selected_role.discard(role_name)
        ...
        print("Selected Role:", self.selected_role)
    ...
    def handle_action(self, action, row):
        ...
        if action == "Edit":
            print(f"Editing Role: {role_name}")
        elif action == "Delete":
            print(f"Deleting Role: {role_name}")

    #------- Row Selection -------
    def select_role(self, row):
        self.table.selectRow(row)
        print(f"Selected Role: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_role(row)