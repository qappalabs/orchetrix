...
class DaemonsetsHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace), 3 (Pods), 5 (Age)
...
class DaemonSetsPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_daemonsets = set()  # Track which deployments are checked
        self.setup_ui()
        self.load_data()
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + Pods + Node Selector + Age +  [Action]
        ...
        custom_header = DaemonsetsHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def load_data(self):
        ...
        Example data:
          ...
          Age: 17h
        """
        self.daemonsets_data = [
            ["kube-proxy", "kube-system", "1", "kubernetes.io/os=linux", "17h"],
        ]
        self.table.setRowCount(len(self.daemonsets_data))

        for row, daemonset in enumerate(self.daemonsets_data):
            self.table.setRowHeight(row, 40)
            name = daemonset[0]
            namespace = daemonset[1]
            pods_str = daemonset[2]
            nodeselector_str = daemonset[3]
            age_str = daemonset[4]
            ...
            nodeselector_value = 0
            try:
                nodeselector_value = float(nodeselector_str)
            except:
                pass
            ...
            item_nodeselector = SortableTableWidgetItem(nodeselector_str, nodeselector_value)
            item_nodeselector.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_nodeselector.setForeground(QColor("#e2e8f0"))
            item_nodeselector.setFlags(item_nodeselector.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_nodeselector)
        ...
        self.items_count.setText(f"{len(self.daemonsets_data)} items")

    #------- Checkbox Helpers -------
    def create_checkbox(self, row, daemonset_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, daemonset_name))
        return checkbox

    def create_checkbox_container(self, row, daemonset_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, daemonset_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, daemonset_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_daemonsets.add(daemonset_name)
        else:
            self.selected_daemonsets.discard(daemonset_name)
        print("Selected deployments:", self.selected_daemonsets)
