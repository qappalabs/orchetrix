...
class CronJobsHeader(QHeaderView):
    """
    ...
    def __init__(self, orientation, parent=None):
        ...
        # Define which columns are sortable: 1 (Name), 2 (Namespace), 4 (Suspend), 5 (Active), 6 (Last sechdule), 7 (Age)
        self.sortable_columns = {1, 2, 4, 5, 6, 7}
...
class CronJobsPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_cronjobs = set()  # Track which deployments are checked
        self.setup_ui()
        self.load_data()
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + Sechdule + Suspend + Activ + Last sechdule+ Age + [Action]
        ...
        custom_header = CronJobsHeader(Qt.Orientation.Horizontal, self.table)
    ...
    def custom_table_mousePressEvent(self, event):
        ...
        if index.isValid() and (index.column() == 0 or index.column() == 8):
    ...
    def load_data(self):
        ...
        Example data:
          ...
          Suspend: 2
          Activ: 3
          Last sechdule: Available Progressing
          Age: 70d
        """
        self.deployments_data = [
            ["coredns", "kube-system", "2/2", "2", "3", "Available Progressing", "70d"],
            ["nginx-deploy", "default", "3/3", "3", "12d", "Available Progressing", "12d"],
        ]
        ...
        for row, deployment in enumerate(self.deployments_data):
            ...
            sechdules_str = deployment[2]
            suspends_str = deployment[3]
            activs_str = deployment[4]
            last_sechdules_str = deployment[5]
            age_str = deployment[6]
            ...
            sechdules_value = 0
            try:
                sechdules_value = float(sechdules_str.split("/")[0])
            except:
                pass

            suspends_value = 0
            try:
                suspends_value = float(suspends_str)
            except:
                pass
            
            last_sechdules_value = 0
            try:
                last_sechdules_value = float(last_sechdules_str)
            except:
                pass
            ...
            # Sechdule -> column 3
            item_sechduls = SortableTableWidgetItem(sechdules_str, sechdules_value)
            item_sechduls.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_sechduls.setForeground(QColor("#e2e8f0"))
            item_sechduls.setFlags(item_sechduls.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_sechduls)

            # Suspend -> column 4
            item_suspends = SortableTableWidgetItem(suspends_str, suspends_value)
            item_suspends.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_suspends.setForeground(QColor("#e2e8f0"))
            item_suspends.setFlags(item_suspends.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, item_suspends)

             # Suspend -> column 5
            item_last_sechdules = SortableTableWidgetItem(last_sechdules_str, last_sechdules_value)
            item_last_sechdules.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_last_sechdules.setForeground(QColor("#e2e8f0"))
            item_last_sechdules.setFlags(item_last_sechdules.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 5, item_last_sechdules)

            # Conditions -> column 6
            item_conditions = QTableWidgetItem(activs_str)
            item_conditions.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
            if "Active" in activs_str:
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, cronjob_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, cronjob_name))
        return checkbox

    def create_checkbox_container(self, row, cronjob_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, cronjob_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, cronjob_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_cronjobs.add(cronjob_name)
        else:
            self.selected_cronjobs.discard(cronjob_name)
        print("Selected cornjobs:", self.selected_cronjobs)
    ...
    def handle_action(self, action, row):
        cronjob_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing cronjobs: {cronjob_name}")
        elif action == "Delete":
            print(f"Deleting cornjobs: {cronjob_name}")

    #------- Row Selection -------
    def select_cronjob(self, row):
        self.table.selectRow(row)
        print(f"Selected cronjobs: {self.table.item(row, 1).text()}")

    def handle_row_click(self, row, column):
        if column != 7:
            self.select_cronjob(row)