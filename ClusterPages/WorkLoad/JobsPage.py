...
class JobsPage(QWidget):
    ...
    def setup_ui(self):
        ...
        # Table: 8 columns total: [Checkbox] + Name + Namespace + Completions + Age + Conditions + [Action]
        ...
        custom_header = JobsHeader(Qt.Orientation.Horizontal, self.table)
        ...
        # Column 3: Completions (stretch)
        self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(3, 100)
        # Column 4: Age (stretch)
        self.table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        # self.table.setColumnWidth(4, 80)
        # Column 5: Conditons (stretch)
    ...
    def load_data(self):
        ...
        for row, sttefulset in enumerate(self.statefulsets_data):
            ...
            completions_str = sttefulset[2]
            age_str = sttefulset[3]
            conditions_str = sttefulset[4]
            ...
            completions_value = 0
            try:
                completions_value = float(completions_str.split("/")[0])
            except:
                pass
            ...
            conditions_value = 0
            try:
                conditions_value = float(conditions_str.replace("d", ""))
            except:
                pass
            ...
            # completions -> column 3
            item_completions = SortableTableWidgetItem(completions_str, completions_value)
            item_completions.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_completions.setForeground(QColor("#e2e8f0"))
            item_completions.setFlags(item_completions.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, item_completions)
            ...
            # conditions -> column 5
            item_conditions = SortableTableWidgetItem(conditions_str, conditions_value)
            ...
            self.table.setItem(row, 5, item_conditions)
    ...
    #------- Checkbox Helpers -------
    def create_checkbox(self, row, job_name):
        checkbox = QCheckBox()
        ...
        """)
        checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, job_name))
        return checkbox

    def create_checkbox_container(self, row, job_name):
        container = QWidget()
        container.setStyleSheet("background-color: transparent;")
        layout = QHBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        checkbox = self.create_checkbox(row, job_name)
        layout.addWidget(checkbox)
        return container

    def handle_checkbox_change(self, state, job_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_replicacontroller.add(job_name)
        else:
            self.selected_replicacontroller.discard(job_name)
        ...
        print("Selected jobs:", self.selected_replicacontroller)
    ...
    def select_job(self, row):
        self.table.selectRow(row)
        print(f"Selected job: {self.table.item(row, 1).text()}")
