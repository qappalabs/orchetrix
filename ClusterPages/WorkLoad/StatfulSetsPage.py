...
"""
Optimized implementation of the statfulsetss page with better memory management
and performance.
"""
...
class StatefulSetsPage(BaseTablePage):
    """
    Displays Kubernetes statfulsets with optimizations for performance and memory usage.
    ...
    def setup_page_ui(self):
        """Set up the main UI elements for the statfulsets page"""
        # Define headers and sortable columns
        headers = ["", "Name", "Namespace", "Pods","Replicas", "Age",""]
        sortable_columns = {1, 2, 4, 5}
        
        # Set up the base UI components
        layout = self.setup_ui("statfulsets", headers, sortable_columns)
    ...
    def load_data(self):
        """Load  statfulsets data into the table with optimized batch processing"""
        # Sample statfulsets data
        statfulsets_data = [
            ["kube-proxy", "kube-system", "1", "kubernetes.io/os=linux", "17h"],
        ]

        # Set up the table for the data
        self.table.setRowCount(len(statfulsets_data))
        
        # Batch process all rows using a single loop for better performance
        for row, statfulsets in enumerate(statfulsets_data):
            self.populate_statfulsets_row(row, statfulsets)
        
        # Update the item count
        self.items_count.setText(f"{len(statfulsets_data)} items")
    
    def populate_statfulsets_row(self, row, statfulsets_data):
        """
        Populate a single row with statfulsets data using efficient methods
        
        Args:
            row: The row index
            statfulsets_data: List containing statfulsets information
        """
        # Set row height once
        self.table.setRowHeight(row, 40)
        
        # Create checkbox for row selection
        statfulsets_name = statfulsets_data[0]
        checkbox_container = self._create_checkbox_container(row, statfulsets_name)
        self.table.setCellWidget(row, 0, checkbox_container)
        
        # Populate data columns efficiently
        for col, value in enumerate(statfulsets_data):
        ...
        self.table.setCellWidget(row, len(statfulsets_data) + 1, action_container)
    ...
    def handle_row_click(self, row, column):
        ...
        if column != self.table.columnCount() - 1:  # Skip action column
            ...
            statfulsets_name = self.table.item(row, 1).text()
            print(f"Selected StatfulSets: {statfulsets_name}")