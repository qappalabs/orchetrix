...
class WorkerSignals(QObject):
    ...
    This class should be a member variable of workers, not a local variable.
    """
...
class KubernetesClient(QObject):
    ...
    def __init__(self):
        ...
        self.metrics_timer = QTimer(self)  # Explicitly set parent to ensure proper cleanup
        self.metrics_timer.timeout.connect(self.refresh_metrics)
        
        self.issues_timer = QTimer(self)  # Explicitly set parent to ensure proper cleanup
    ...
    def __del__(self):
        ...
        try:
            ...
            # Clear QThreadPool - wait for jobs to finish
            if hasattr(self, 'threadpool'):
                # Set maximum wait time to 500ms to prevent hanging during shutdown
    ...
    def load_clusters_async(self, config_path=None):
        ...
        # Set up cleanup when worker is done
    ...
    def get_cluster_info(self, cluster_name):
        ...
        try:
            ...
            # Only emit signal if not shutting down
            if not self._shutting_down:
    ...
    def get_cluster_metrics_async(self):
        ...
        # Connect signals safely with checks for None in case of shutdown
    ...
    def get_node_metrics_async(self, node_name):
        ...
        worker.signals.finished.connect(
            lambda result: self.node_metrics_updated.emit(result) if result and not self._shutting_down else None
        )
        worker.signals.error.connect(
            lambda error: self.error_occurred.emit(error) if not self._shutting_down else None
        )
