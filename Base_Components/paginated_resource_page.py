...
class PaginatedResourcePage(BaseResourcePage):
    ...
    def setup_pagination_ui(self):
        ...
        self.pagination_controls.update_status(0, self.has_more_data, False)
    ...
    def _start_data_loading(self, load_more: bool):
        ...
        unified_loader.loading_completed.disconnect(self._on_unified_data_loaded)
        unified_loader.loading_error.disconnect(self._on_unified_data_error)
        ...
        # Start loading
        unified_loader.load_resources_async(
    ...
    def _on_data_loaded(self, result):
        ...
        try:
            ...
            if not resources:
                # No data received
                self._finish_loading(load_more=self.total_loaded > 0)
                return

            # Process data in background
            self._start_data_processing(resources, load_more=self.total_loaded > 0)
    ...
    def _start_data_processing(self, raw_resources: List[Dict], load_more: bool):
        ...
        if self.processing_worker:
            try:
                self.processing_worker.data_processed.disconnect()
                self.processing_worker.progress_updated.disconnect()
                self.processing_worker.error_occurred.disconnect()
                if hasattr(self.processing_worker, 'stop'):
                    self.processing_worker.stop()
                elif hasattr(self.processing_worker, 'cancel'):
                    self.processing_worker.cancel()
            except Exception as e:
                logging.debug(f"Error disconnecting signals from previous worker: {e}")
            self.processing_worker = None
