...
class VirtualizedResourceModel(QAbstractTableModel):
    ...
    Only formats data when actually displayed, rendering is virtualized with on-demand formatting without dirty-flag-based caching.
    """
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole):
        """Return data for given index and role - values are formatted lazily on demand when displayed, with no caching; out-of-range or invalid indexes return None and are validated using self._data"""
    ...
    def mark_all_dirty(self):
        ...
        self.endResetModel()
        logging.debug("Forced complete model refresh via mark_all_dirty")
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        """DEPRECATED: Get cache performance statistics - This method always returns zeros for backward compatibility."""
        warnings.warn("get_cache_stats is deprecated and always returns zeros.", DeprecationWarning, stacklevel=2)
        return {
            'cache_hits': 0,
            'cache_misses': 0,
            'hit_rate_percent': 0,
