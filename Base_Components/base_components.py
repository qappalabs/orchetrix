    ...
    def __init__(self, status_text, color=None, parent=None):
        ...
        if color:
            self.label.setStyleSheet(
                BaseComponentsStyles.get_status_badge_label_style(color))
...
class CustomHeader(QHeaderView):
    ...
    def paintEvent(self, event):
        """Custom paint event for sort indicators."""
...
class BaseTablePage(ThemeAwareMixin, QWidget):
    ...
    def eventFilter(self, obj, event):
        """Filter events to handle clicks outside table."""
    ...
    def _on_theme_changed(self, theme_name):
        ...
        if hasattr(self, 'table') and self.table:
            ...
            # Consolidated single-pass row iteration for efficiency
            action_col = self.table.columnCount() - 1
            for row in range(self.table.rowCount()):
                # (a) Clear all item backgrounds so theme-aware stylesheet colors apply
                for col in range(self.table.columnCount()):
                    item = self.table.item(row, col)
                    if item:
                ...
                # (b) Refresh row checkbox (column 0)
                checkbox_container = self.table.cellWidget(row, 0)
                if checkbox_container:
                ...
                # (c) Refresh action button and icon (last column)
                ...
                if action_container:
                    ...
                    for child in action_container.children():
                        if isinstance(child, QToolButton):
                            # Update button style
                            child.setStyleSheet(
                                BaseTablePageStyles.get_action_button_style())
        ...
        if hasattr(self, '_item_widgets') and 'header_widget' in self._item_widgets:
            ...
            if header_widget:
                header_widget.setStyleSheet(
                    BaseTablePageStyles.get_header_widget_style())

        logging.debug(
            f"BaseTablePage: Theme refresh complete for {theme_name}")
