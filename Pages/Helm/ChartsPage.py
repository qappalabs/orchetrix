...
class ChartDataThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if self.is_interrupted():
                logging.info("ChartDataThread: Interrupted before emit, skipping signal")
                return
            ...
            # Check for cancellation before emitting error
            if self.is_interrupted():
                logging.info("ChartDataThread: Interrupted during error handling, skipping signal")
                return
    ...
    def process_api_data(self, api_data, session, headers):
        ...
        for package in packages:
            try:
                ...
                chart_id = package.get('name', package.get('package_id', 'unknown'))
                logging.debug(
                    f"Failed to process chart '{chart_id}': {e}",
                    exc_info=True
                )
    ...
    def download_icon(self, package, session, headers):
        ...
        try:
            ...
            # Check for cancellation before network I/O
            if self.is_interrupted():
            ...
            if icon_url:
                ...
                if not os.path.exists(icon_path):
                    try:
                        ...
                        logging.debug(
                            f"Icon download failed for chart '{package_name}': {e}",
                            exc_info=True
                        )
                        icon_path = None
            ...
            logging.debug(
                f"Icon processing failed for chart '{package_name}': {e}",
                exc_info=True
            )
            icon_path = None
...
class ChartsPage(BaseResourcePage):
    ...
    def on_data_loaded(self, data, is_more_available):
        ...
        if hasattr(self, 'is_searching') and self.is_searching:
            ...
            self.is_loading = False # Ensure we reset loading state
            return
    ...
    def on_search_data_loaded(self, data, is_more_available):
        ...
        if data:
            ...
            self.items_count.setText(f"{len(data)} results for '{search_term}'")
