...
class OrchestrixGUI(QMainWindow):
    ...
    def add_table_item(self, name, kind, source, label, status, badge_color=None, original_data=None):
        ...
        if original_data and 'cluster_data' in original_data:
            ...
            pin_btn.setIcon(QIcon("icons/pin.png") if name not in self.pinned_items else QIcon("icons/unpin.png"))
    ...
    def toggle_pin_item(self, name):
        ...
        if not data_item or 'cluster_data' not in data_item:
            print(f"Cannot pin {name}: No cluster_data found")  # Debug print
            return
        
        if name in self.pinned_items:
            self.pinned_items.remove(name)
            pin_icon = QIcon("icons/pin.png")
            action = "unpinned"
        else:
            self.pinned_items.add(name)
            pin_icon = QIcon("icons/unpin.png")
            action = "pinned"
        
        print(f"Item {name} {action}. Pinned items now: {self.pinned_items}")  # Debug print
        ...
        print(f"Emitting updated pinned items from HomePage: {pinned_list}")  # Debug print
