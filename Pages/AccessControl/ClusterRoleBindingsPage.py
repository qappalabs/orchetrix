"""
Dynamic implementation of the Cluster Role Bindings page with live Kubernetes data.
"""
...
class ClusterRoleBindingsPage(BaseResourcePage):
    """
    Displays Kubernetes cluster role bindings with live data and resource operations.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.resource_type = "clusterrolebindings"  # Set resource type for kubectl
        self.show_namespace_dropdown = False  # ClusterRoleBindings are cluster-scoped
        self.setup_page_ui()
    ...
    def populate_resource_row(self, row, resource):
        """
        Populate a single row with cluster role binding data from live Kubernetes resources
        """
        # Set row height once
        ...
        # Format the bindings (subjects) names
        subjects = resource.get("raw_data", {}).get("subjects", [])
        binding_names = []
        ...
        for subject in subjects:
            ...
            if kind and name:
                binding_names.append(f"{kind}/{name}")

        bindings_text = ", ".join(binding_names) if binding_names else "<none>"
