...
class OverviewDataWorker(QThread):
    ...
    def is_interrupted(self):
        ...
        return self._should_stop or is_shutdown_requested()

    def run(self):
        """
        Load overview data directly without nested threads.
        The QThread already provides background execution - no need for nested QThread.
        """
        try:
            ...
            try:
                ...
                if hasattr(self.kube_client, 'version_api'):
                    version = self.kube_client.version_api.get_code(
                        _request_timeout=APIClientConfig.VERSION_CHECK_TIMEOUT)
                    logging.info(
                        f"Connected to Kubernetes cluster version: {version.git_version if version else 'Unknown'}")
                else:
                    logging.warning(
                        "Cannot test connection - proceeding with data load")
            except Exception as conn_e:
                logging.warning(
                    f"Connection test failed: {conn_e}, but continuing with data load")
            ...
            # Load overview data directly - no nested thread needed
            # The QThread we're running in already provides background execution
    ...
    def _load_workloads_summary(self):
        ...
        try:
            ...
            def safe_count_resources(resource_type, api_call, progress_pct, progress_msg):
                ...
                try:
                    resources = api_call()
                    count = len(resources.items) if hasattr(
                        resources, 'items') else 0
                    logging.info(
                        f"OverviewPage: Loaded {count} {resource_type}")
                    return count
                except Exception as e:
                    logging.warning(
                        f"Error loading {resource_type}: {e}")
                    return 0
            ...
            workloads_data['deployments'] = safe_count_resources(
                'deployments',
                lambda: api_service.apps_v1.list_deployment_for_all_namespaces(
                    limit=5000),
            ...
            workloads_data['daemonsets'] = safe_count_resources(
                'daemonsets',
                lambda: api_service.apps_v1.list_daemon_set_for_all_namespaces(
                    limit=5000),
            ...
            workloads_data['statefulsets'] = safe_count_resources(
                'statefulsets',
                lambda: api_service.apps_v1.list_stateful_set_for_all_namespaces(
                    limit=5000),
            ...
            workloads_data['replicasets'] = safe_count_resources(
                'replicasets',
                lambda: api_service.apps_v1.list_replica_set_for_all_namespaces(
                    limit=5000),
            ...
            workloads_data['jobs'] = safe_count_resources(
                'jobs',
                lambda: api_service.batch_v1.list_job_for_all_namespaces(
                    limit=5000),
                60, "Loading jobs..."
            )
            ...
            try:
                workloads_data['cronjobs'] = safe_count_resources(
                    'cronjobs',
                    lambda: api_service.batch_v1.list_cron_job_for_all_namespaces(
                        limit=5000),
                    70, "Loading cron jobs..."
                )
            except Exception as e:
                logging.warning(
                    f"Error loading cronjobs: {e}")
            ...
            try:
                if check_timeout() or self.is_interrupted():
                    ...
                    while batch_count < max_batches and not (check_timeout() or self.is_interrupted()):
                        try:
                            ...
                            self.progress_update.emit(
                                progress, f"Loading pods... ({pods_total} found)")

                            # Check for next page
                            continue_token = getattr(pods_batch.metadata, 'continue', None) if hasattr(
                                pods_batch, 'metadata') else None
                            if not continue_token:
                                break

                        except Exception as batch_e:
                            logging.warning(
                                f"Error in pods batch {batch_count}: {batch_e}")
                            break

                    workloads_data['pods_total'] = pods_total
                    workloads_data['pods_running'] = pods_running
                    logging.info(
                        f"OverviewPage: Loaded {pods_total} pods ({pods_running} running) in {batch_count} batches")
            ...
            logging.info(
                f"OverviewPage: Loaded workloads summary in {load_time:.1f}ms")
...
class MetricCard(QWidget):
    ...
    def __init__(self, title, resource_type, parent=None):
        super().__init__(parent)
    ...
    def setup_ui(self, title):
        ...
        self.title_label.setStyleSheet(
            OverviewPageStyles.get_title_label_style())
        ...
        self.metric_label.setStyleSheet(
            OverviewPageStyles.get_metric_label_style())
        ...
        self.subtitle_label.setStyleSheet(
            OverviewPageStyles.get_subtitle_label_style())
    ...
    def update_data(self, running, total):
        ...
        logging.info(
            f"MetricCard: Updating {self.resource_type} card with data: {running}/{total}")
        ...
        self.subtitle_label.setText(
            f"Running / Total {self.title_label.text()}")

        # Force a repaint to ensure the update is visible
        self.metric_label.update()
        self.subtitle_label.update()

        # Use different styling for progressive vs final display - commented out for now

        self._card_state = 'normal'
        self.card.setStyleSheet(OverviewPageStyles.get_metric_card_normal_style())

        logging.info(
            f"MetricCard: Successfully updated {self.resource_type} card display")

    def show_loading(self):

        self._card_state = 'loading'
        self.metric_label.setText("⏳")
        self.subtitle_label.setText("Loading...")
        self.card.setStyleSheet(
            OverviewPageStyles.get_metric_card_loading_style())
        self.metric_label.update()
        self.subtitle_label.update()
    ...
    def show_error_state(self, show_error, error_message=""):
        ...
        if show_error:
            self._card_state = 'error'
            self.metric_label.setText("Error")
            self.subtitle_label.setText(error_message)
            self.card.setStyleSheet(
                OverviewPageStyles.get_metric_card_error_style())
        else:
            # Clear error state and show data
            self._card_state = 'normal'
            self.metric_label.setText(f"{self.running} / {self.total}")
            self.subtitle_label.setText(
                f"Running / Total {self.resource_type.title()}")
            self.card.setStyleSheet(
                OverviewPageStyles.get_metric_card_normal_style())

    def refresh_styles(self):

        try:
            # Refresh card style while preserving current state
            if hasattr(self, 'card') and self.card:
                state = getattr(self, '_card_state', 'normal')
...
class OverviewPage(ThemeAwarePage):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            if self.initialize_kube_client():
                ...
                QTimer.singleShot(500, lambda: self.show_connection_error(
    ...
    def setup_refresh_timer(self):
        ...
        self.refresh_timer = None
        logging.info(
            "OverviewPage: Auto - refresh disabled - manual refresh only")
    ...
    def setup_ui(self):
        ...
        for title, resource_type in card_configs:
            ...
            card.setSizePolicy(QSizePolicy.Policy.Fixed,
        ...
        # Store scroll area as instance attribute
        self.scroll_areas = [scroll_area]
    ...
    def _on_theme_changed(self, theme_name):
        ...
        try:
            logging.info(
                f"OverviewPage: Theme changed to {theme_name}, refreshing styles")
            ...
            if title_label:
                title_label.setStyleSheet(
                    OverviewPageStyles.get_page_title_style())

            # Refresh scroll area
            if hasattr(self, 'scroll_areas') and self.scroll_areas:
                for scroll_area in self.scroll_areas:
                    scroll_area.setStyleSheet(
                        OverviewPageStyles.get_scroll_area_style())
    ...
    def force_load_data(self):
        ...
        if self._loading_in_progress:
            logging.info(
                "OverviewPage: Force loading data requested - already in progress, skipping")
            return
    ...
    def fetch_kubernetes_data(self):
        ...
        try:
            ...
            if self._loading_in_progress:
                logging.info(
                    "OverviewPage: Background loading already in progress, skipping")
                return
            ...
            # Don't block on is_connected() check - let the background worker handle
            # connection testing. The worker has proper timeout handling and will
            # report errors via the error_occurred signal if connection fails.
            ...
            self.data_worker.data_loaded.connect(
                self._on_background_data_loaded)
            ...
            logging.error(
                f"OverviewPage: Error starting background data fetch: {e}")
            self._loading_in_progress = False
            self.show_connection_error(
                f"Failed to start data loading: {str(e)}")

    def _on_background_data_loaded(self, overview_data):

        try:
            logging.info(
                f"OverviewPage: Received overview data from background worker: {overview_data}")
            ...
            if 'workloads' in overview_data:
                ...
                logging.warning(
                    "OverviewPage: No workloads data in overview_data")
            ...
            logging.info(
                "OverviewPage: Successfully updated all overview cards")
    ...
    def _on_background_error(self, error_message):
        ...
        try:
            logging.error(
                f"OverviewPage: Background worker error: {error_message}")
            ...
            logging.error(
                f"OverviewPage: Error handling background error: {e}")
    ...
    def _on_worker_finished(self):
        ...
        try:
            ...
            logging.error(
                f"OverviewPage: Error in worker finished handler: {e}")

    def _update_workloads_cards(self, workloads_data):

        try:
            logging.info(
                f"OverviewPage: Updating cards with data: {workloads_data}")
            ...
            if 'deployments' in self.metric_cards:
                ...
                self.metric_cards['deployments'].update_data(
            ...
            if 'daemonsets' in self.metric_cards:
                ...
                self.metric_cards['daemonsets'].update_data(
            ...
            if 'cronjobs' in self.metric_cards:
                ...
                self.metric_cards['cronjobs'].update_data(
    ...
    def initialize_kube_client(self):
        ...
        try:
            ...
            if self.kube_client:
                # Just check if we have a client - don't block on is_connected()
                # The actual data loading will handle connection issues gracefully
                cluster_name = getattr(
                    self.kube_client, 'current_cluster', None)
                if cluster_name:
                    logging.info(
                        f"OverviewPage: Kubernetes client initialized for cluster: {cluster_name}")
                else:
                    logging.info(
                        "OverviewPage: Kubernetes client initialized (no cluster selected yet)")
                # Always return True if we have a client - let data loading handle connection
                return True
            else:
                logging.warning(
                    "OverviewPage: Kubernetes client not properly initialized")
                self.kube_client = None
                return False
        except Exception as e:
            logging.error(
                f"OverviewPage: Failed to initialize Kubernetes client: {e}")
            self.kube_client = None
            return False
    ...
    def showEvent(self, event):
        ...
        if not self._loading_in_progress:
            ...
            if all_empty:
                logging.info(
                    "OverviewPage: Show event detected empty cards, triggering data load")
