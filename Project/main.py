...
# import sys
# from PyQt6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QStackedWidget, QLabel, QSplashScreen, QTableWidget
# from PyQt6.QtCore import Qt, QTimer, QEvent, QSize
...
#     def __init__(self):
#         super().__init__()
#         # Initialize Kubernetes client first
...
#         # Load Kubernetes configuration after UI is set up
...
#         self.kubernetes_client.load_clusters_async()
        
#         # Connect signal to handle loaded clusters
#         self.kubernetes_client.clusters_loaded.connect(self.on_clusters_loaded)
        
#         # Connect error signal
#         self.kubernetes_client.error_occurred.connect(self.handle_kubernetes_error)
...
#         # Update home page with available clusters
#         if hasattr(self.home_page, 'update_clusters'):
#             self.home_page.update_clusters(clusters)
    
#     def handle_kubernetes_error(self, error_message):
#         """Handle Kubernetes client errors"""
#         print(f"Kubernetes error: {error_message}")
#         # You could display this in a status bar or dialog
...
#     def show_detail_for_table_item(self, row, col, page, page_name):
#         """Show detail page for clicked table item and fetch its data"""
#         # Get resource type from page name (remove trailing 's' if present)
#         resource_type = page_name.rstrip('s')  # Convert "Pods" to "Pod", etc.
...
#         # Show detail panel
#         if resource_name:
#             # Get the namespace (might be available in the table data)
#             namespace = "default"
#             if hasattr(page.table, 'item') and page.table.columnCount() > 2:
#                 # Check if there's a namespace column
#                 for col_idx in range(page.table.columnCount()):
#                     header_item = page.table.horizontalHeaderItem(col_idx)
#                     if header_item and header_item.text().lower() == "namespace":
#                         namespace_item = page.table.item(row, col_idx)
#                         if namespace_item:
#                             namespace = namespace_item.text()
#                             break
            
#             # Fetch resource data first
#             self.fetch_resource_details(resource_type, resource_name, namespace)
            
#             # Then show the detail page
#             self.detail_page.show_detail(resource_type, resource_name)
    
#     def fetch_resource_details(self, resource_type, resource_name, namespace="default"):
#         """Fetch detailed resource information via Kubernetes client"""
#         # This asynchronous call will emit a signal with the resource data
#         self.kubernetes_client.get_resource_detail_async(resource_type.lower(), resource_name, namespace)
        
#         # Connect detail signal to update the detail page (only once)
#         try:
#             self.kubernetes_client.resource_detail_loaded.disconnect(self.detail_page.update_resource_data)
#         except:
#             pass  # Ignore if no connections exist
        
#         # Connect the signal to update the detail page
#         self.kubernetes_client.resource_detail_loaded.connect(self.detail_page.update_resource_data)
...
#             # Stop any active metric polling when leaving cluster view
#             if self.kubernetes_client:
#                 self.kubernetes_client.stop_metrics_polling()
#                 self.kubernetes_client.stop_issues_polling()
...
#         # Tell kubernetes client to switch context and get cluster info
#         self.kubernetes_client.get_cluster_info(cluster_name)
        
#         # Switch to cluster view
#         self.cluster_view.set_active_cluster(cluster_name)
#         self.stacked_widget.setCurrentWidget(self.cluster_view)
        
#         # Start metrics polling when entering cluster view
#         self.kubernetes_client.start_metrics_polling()
#         self.kubernetes_client.start_issues_polling()
...
#             # Stop any active metric polling when leaving cluster view
#             if self.kubernetes_client and self.previous_page == self.cluster_view:
#                 self.kubernetes_client.stop_metrics_polling()
#                 self.kubernetes_client.stop_issues_polling()
...
#             # Restart metrics polling if returning to cluster view
#             if self.previous_page == self.cluster_view and self.kubernetes_client:
#                 self.kubernetes_client.start_metrics_polling()
#                 self.kubernetes_client.start_issues_polling()
#         else:
#             # Default to home page if there's no previous page
...
#         # If switching to cluster view, ensure metrics polling is active
#         if current_widget == self.cluster_view and self.kubernetes_client:
#             self.kubernetes_client.start_metrics_polling()
#             self.kubernetes_client.start_issues_polling()
#         # If switching away from cluster view, stop metrics polling
#         elif self.kubernetes_client:
#             self.kubernetes_client.stop_metrics_polling()
#             self.kubernetes_client.stop_issues_polling()
