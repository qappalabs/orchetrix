...
class ClusterConnection(QObject):
    ...
    def connect_to_cluster(self, cluster_name):
        ...
        class ConnectionWorker(QRunnable):
            def __init__(self, client, cluster_name, parent):
                ...
                self.parent = parent
                self.signals = WorkerSignals()
                self.signals.finished.connect(self.parent.on_connection_complete)
                self.signals.error.connect(self.parent.handle_error)
            ...
            def run(self):
                try:
                    ...
                    try:
                        ...
                        if result.returncode != 0:
                            self.signals.error.emit(f"Failed to switch context: {result.stderr}")
                            self.signals.finished.emit((self.cluster_name, False))
                            return
                        ...
                        if result.returncode != 0:
                            self.signals.error.emit(f"Failed to get cluster info: {result.stderr}")
                            self.signals.finished.emit((self.cluster_name, False))
                            return
                            
                        # Successful connection
                        self.signals.finished.emit((self.cluster_name, True))
                    except subprocess.TimeoutExpired:
                        self.signals.error.emit(f"Timeout connecting to cluster: {self.cluster_name}")
                        self.signals.finished.emit((self.cluster_name, False))
                    except Exception as e:
                        self.signals.error.emit(f"Error connecting to cluster: {str(e)}")
                        self.signals.finished.emit((self.cluster_name, False))
                except Exception as e:
                    self.signals.error.emit(f"Error connecting to cluster: {str(e)}")
                    self.signals.finished.emit((self.cluster_name, False))
        ...
        # Create and start the worker
        worker = ConnectionWorker(self.kube_client, cluster_name, self)
        self.threadpool.start(worker)
    ...
    def load_nodes(self):
        ...
        class NodesWorker(QRunnable):
            def __init__(self, client, parent):
                ...
                self.parent = parent
                self.signals = WorkerSignals()
                self.signals.finished.connect(self.parent.on_nodes_loaded)
                self.signals.error.connect(self.parent.handle_error)
            ...
            def run(self):
                try:
                    ...
                    if result.returncode != 0:
                        self.signals.error.emit(f"Failed to get nodes: {result.stderr}")
                    ...
                    self.signals.finished.emit(nodes)
                except Exception as e:
                    self.signals.error.emit(f"Error loading nodes: {str(e)}")
                    self.signals.finished.emit([])
    ...
    def load_metrics(self):
        """Load cluster metrics with better formatting and units"""
        try:
