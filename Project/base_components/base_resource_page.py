...
class BaseResourcePage(BaseTablePage):
    ...
    def setup_ui(self, title, headers, sortable_columns=None):
        ...
        # Make the content reload automatically on page selection/activation
    ...
    def showEvent(self, event):
        ...
        # Load data if needed
        if hasattr(self, 'reload_on_show') and self.reload_on_show:
            self.load_data()
    ...
    def on_resources_loaded(self, resources, resource_type):
        """Handle the loaded resources with improved empty state display."""
        self.is_loading = False
        
        # Important: Clear the table completely first
        self.table.setRowCount(0)
        ...
        if not resources:
            # Create exactly one row for the empty state
            self.table.setRowCount(1)
            
            # Important: Span this row across all columns
            self.table.setSpan(0, 0, 1, self.table.columnCount())
            
            # Create a container widget for our empty state message
            ...
            # Create the empty state message with larger, more visible text
            empty_text = QLabel(f"No {self.resource_type} found")
            ...
            empty_description = QLabel(f"The list is empty. No {self.resource_type} exist in the current context.")
            ...
            # Add a refresh button in the empty state
            ...
            empty_layout.addWidget(refresh_button, 0, Qt.AlignmentFlag.AlignCenter)
            
            # Important: Set the empty widget in the cell
        ...
        # Ensure table repaints to show empty state
        self.table.repaint()
