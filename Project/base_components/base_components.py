...
class BaseTablePage(QWidget):
    ...
    def setup_ui(self, title, headers, sortable_columns=None):
        ...
        self.table.mousePressEvent = self._custom_table_mousePressEvent
    ...
    def _custom_table_mousePressEvent(self, event):
        """Custom mouse handler without toggling the checkbox on row click"""
        index = self.table.indexAt(event.pos())
        if index.isValid():
            row = index.row()
            # Removed code that toggles the checkbox
            # Optionally, you can still select the row if needed
    ...
    def _create_action_button(self, row, actions=None):
