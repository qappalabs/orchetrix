...
class PortForwardingPage(BaseResourcePage):
    ...
    def __init__(self, parent=None):
        ...
        # Port forwarding is not a standard k8s resource type
        # So we don't set self.resource_type = "portforward"
    ...
    def load_data(self):
        ...
        try:
            ...
            for key, process_info in self.port_forward_processes.items():
                if not process_info['process'].isRunning():
            ...
            # Update the UI with found resources
            self.resources = port_forwards
            
            # Update the table
            self.table.setRowCount(len(port_forwards))
            
            # Populate the table
            for row, port_forward in enumerate(port_forwards):
                self.populate_resource_row(row, port_forward)
            
            # Update the item count
            self.items_count.setStyleSheet(AppStyles.ITEMS_COUNT_STYLE)
            self.items_count.setText(f"{len(port_forwards)} items")
            
            # Re-enable sorting
            self.table.setSortingEnabled(True)
            
            # If no port forwards, show empty state
            if not port_forwards:
                self.show_empty_state("No port forwarding found")
            
        except Exception as e:
            self.on_load_error(f"Error loading port forwarding data: {str(e)}")
            
    def add_loading_indicator(self):
        """Add a loading indicator row to the table"""
        ...
        loading_text.setStyleSheet("color: #BBBBBB;")
    ...
    def show_empty_state(self, message):
        """Show empty state message when no resources found"""
        ...
        empty_text.setStyleSheet("color: #888888; font-size: 16px; font-weight: bold;")
        
        empty_description = QLabel("No active port forwarding configurations found")
        empty_description.setAlignment(Qt.AlignmentFlag.AlignCenter)
        empty_description.setStyleSheet("color: #666666; font-size: 14px; margin-top: 5px;")
    ...
    def populate_resource_row(self, row, resource):
        ...
        for col, value in enumerate(columns):
            ...
            # Set text alignment
            if col in [2, 3, 4, 5, 6]:  # Center-align Kind, Pod Port, Local Port, Protocol, Status
            ...
            # Set special colors for status column - exactly like ServicesPage
    ...
    def delete_resource(self, resource_name, resource_namespace):
        ...
        if key in self.port_forward_processes:
            ...
            if process_info["process"].state() == QProcess.ProcessState.Running:
    ...
    def delete_selected_resources(self):
        ...
        for resource in self.resources:
            if resource["name"] in self.selected_items:
        ...
        QMessageBox.information(
            ...
            f"{count} port forwards have been deleted."
        )
    ...
    def on_load_error(self, error_message):
        ...
        # Log the error
        print(f"Error loading port forwards: {error_message}")