"""
Optimized implementation of the Roles page with better memory management
and performance.
"""
...
class RolesPage(BaseTablePage):
    """
    Displays Kubernetes roles with optimizations for performance and memory usage.
    ...
    def setup_page_ui(self):
        ...
        layout = self.setup_ui("Roles", headers, sortable_columns)
    ...
    def load_data(self):
        """Load role data into the table with optimized batch processing"""
        # Sample role data
        roles_data = [
        ...
        self.table.setRowCount(len(roles_data))
        ...
        self.items_count.setText(f"{len(roles_data)} items")
    
    def populate_role_row(self, row, role_data):
        """
        Populate a single row with role data using efficient methods
        
        Args:
            row: The row index
            role_data: List containing role information
        """
        # Set row height once
    ...
    def _handle_action(self, action, row):
        """Override base action handler for role-specific actions"""
