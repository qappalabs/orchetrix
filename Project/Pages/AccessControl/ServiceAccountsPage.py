"""
Optimized implementation of the Service Accounts page with better memory management
and performance.
"""
...
class ServiceAccountsPage(BaseTablePage):
    """
    Displays Kubernetes service accounts with optimizations for performance and memory usage.
    ...
    def load_data(self):
        """Load service account data into the table with optimized batch processing"""
        ...
        self.table.setRowCount(len(service_accounts_data))
    ...
    def populate_account_row(self, row, account_data):
        """
        Populate a single row with service account data using efficient methods
        
        Args:
            row: The row index
            account_data: List containing service account information
        """
        # Set row height once
        ...
        for col, value in enumerate(account_data):
    ...
    def _handle_action(self, action, row):
        """Override base action handler for service account-specific actions"""
        account_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing Service Account: {account_name}")
        elif action == "Delete":
            print(f"Deleting Service Account: {account_name}")
    ...
    def handle_row_click(self, row, column):
        ...
        if column != self.table.columnCount() - 1:  # Skip action column
            ...
            # Log selection
            account_name = self.table.item(row, 1).text()
            print(f"Selected Service Account: {account_name}")
