...
# class ReplicaControllersHeader(QHeaderView):
#     """
...
#         # Define which columns are sortable: 1 (Name), 2 (Namespace), 3 (Replicas), 4 (Desired Replicas), 4 (Selector)
...
# class ReplicaControllersPage(QWidget):
#     def __init__(self, parent=None):
#         super().__init__(parent)
#         self.selected_replicacontroller = set()  # Track which ReplicaControlers are checked
...
#         title = QLabel("Replica Controllers")
#         title.setStyleSheet("""
#             font-size: 20px;
#             font-weight: bold;
#             color: #ffffff;
...
#         # Table: 8 columns total: [Checkbox] + Name + Namespace + Replicas + Desored Replicas + Selector + [Action]
...
#         custom_header = ReplicaControllersHeader(Qt.Orientation.Horizontal, self.table)
...
#         # Column 3: Replicas (stretch)
#         self.table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
#         # self.table.setColumnWidth(3, 100)
#         # Column 4: Desired Replicas (stretch)
#         self.table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
#         # self.table.setColumnWidth(4, 80)
#         # Column 5: Selector (stretch)
...
#           Replcas: 1
#           Desired Replicas: kubernetes.io/os=linux
#           Selector: 17h
#         """
...
#             replicas_str = sttefulset[2]
#             desired_replicas_str = sttefulset[3]
#             selector_str = sttefulset[4]
...
#             replicas_value = 0
#             try:
#                 replicas_value = float(replicas_str.split("/")[0])
#             except:
#                 pass

#             desired_replicas_value = 0
#             try:
#                 desired_replicas_value = float(desired_replicas_str)
#             except:
#                 pass

#             selector_value = 0
#             try:
#                 selector_value = float(selector_str.replace("d", ""))
#             except:
#                 pass
...
#             self.table.setItem(row, 3, item_replicas)

#             # Deisred Replicas -> column 4
#             item_desired_replicas = SortableTableWidgetItem(desired_replicas_str, desired_replicas_value)
#             item_desired_replicas.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
#             item_desired_replicas.setForeground(QColor("#e2e8f0"))
#             item_desired_replicas.setFlags(item_desired_replicas.flags() & ~Qt.ItemFlag.ItemIsEditable)
#             self.table.setItem(row, 4, item_desired_replicas)

#             # Selector -> column 5
#             item_selector = SortableTableWidgetItem(selector_str, selector_value)
#             item_selector.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
#             item_selector.setForeground(QColor("#e2e8f0"))
#             item_selector.setFlags(item_selector.flags() & ~Qt.ItemFlag.ItemIsEditable)
#             self.table.setItem(row, 5, item_selector)
...
#     #------- Checkbox Helpers -------
#     def create_checkbox(self, row, replicacontroller_name):
#         checkbox = QCheckBox()
...
#             }
#         """)
#         checkbox.stateChanged.connect(lambda s: self.handle_checkbox_change(s, replicacontroller_name))
#         return checkbox

#     def create_checkbox_container(self, row, replicacontroller_name):
#         container = QWidget()
#         container.setStyleSheet("background-color: transparent;")
#         layout = QHBoxLayout(container)
#         layout.setContentsMargins(0, 0, 0, 0)
#         layout.setAlignment(Qt.AlignmentFlag.AlignCenter)
#         checkbox = self.create_checkbox(row, replicacontroller_name)
#         layout.addWidget(checkbox)
#         return container

#     def handle_checkbox_change(self, state, replicacontroller_name):
#         if state == Qt.CheckState.Checked.value:
#             self.selected_replicacontroller.add(replicacontroller_name)
#         else:
#             self.selected_replicacontroller.discard(replicacontroller_name)
...
#         print("Selected replicacontrollers:", self.selected_replicacontroller)
...
#         if action == "Edit":
#             print(f"Editing replicacontroller: {daemonsets_name}")
#         elif action == "Delete":
#             print(f"Deleting replicacontroller: {daemonsets_name}")

#     #------- Row Selection -------
#     def select_replicacontroller(self, row):
#         self.table.selectRow(row)
#         print(f"Selected replicacontroller: {self.table.item(row, 1).text()}")

#     def handle_row_click(self, row, column):
#         if column != 7:
#             self.select_replicacontroller(row)


"""
Optimized implementation of the replication_controlerss page with better memory management
and performance.
"""
...
class ReplicaControllersPage(BaseTablePage):
    """
    Displays Kubernetes replication_controlers with optimizations for performance and memory usage.
    ...
    def setup_page_ui(self):
        """Set up the main UI elements for the replication_controlers page"""
        # Define headers and sortable columns
        headers = ["", "Name", "Namespace", "Replicas","Desired Replicas", "Selector",""]
        sortable_columns = {1, 2, 3, 4, 5}
        
        # Set up the base UI components
        layout = self.setup_ui("Replication Controlers", headers, sortable_columns)
    ...
    def load_data(self):
        """Load  replication_controlers data into the table with optimized batch processing"""
        # Sample replication_controlers data
        replication_controlers_data = [
            ["kube-proxy", "kube-system", "1", "kubernetes.io/os=linux", "17h"],
        ]

        # Set up the table for the data
        self.table.setRowCount(len(replication_controlers_data))
        
        # Batch process all rows using a single loop for better performance
        for row, replication_controlers in enumerate(replication_controlers_data):
            self.populate_replication_controlers_row(row, replication_controlers)
        
        # Update the item count
        self.items_count.setText(f"{len(replication_controlers_data)} items")
    
    def populate_replication_controlers_row(self, row, replication_controlers_data):
        """
        Populate a single row with replication_controlers data using efficient methods
        
        Args:
            row: The row index
            replication_controlers_data: List containing replication_controlers information
        """
        # Set row height once
        self.table.setRowHeight(row, 40)
        
        # Create checkbox for row selection
        replication_controlers_name = replication_controlers_data[0]
        checkbox_container = self._create_checkbox_container(row, replication_controlers_name)
        self.table.setCellWidget(row, 0, checkbox_container)
        
        # Populate data columns efficiently
        for col, value in enumerate(replication_controlers_data):
            cell_col = col + 1  # Adjust for checkbox column
            
            # Handle numeric columns for sorting
            if col == 2:  # Replicas column
        ...
        self.table.setCellWidget(row, len(replication_controlers_data) + 1, action_container)
    ...
    def handle_row_click(self, row, column):
        ...
        if column != self.table.columnCount() - 1:  # Skip action column
            ...
            replication_controlers_name = self.table.item(row, 1).text()
            print(f"Selected replication_controlers: {replication_controlers_name}")
