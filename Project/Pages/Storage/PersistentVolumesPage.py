"""
Optimized implementation of the Persistent Volumes page with better memory management
and performance.
"""
...
class PersistentVolumesPage(BaseTablePage):
    """
    Displays Kubernetes persistent volumes with optimizations for performance and memory usage.
    ...
    def load_data(self):
        ...
        self.table.setRowCount(len(pv_data))
    ...
    def populate_pv_row(self, row, pv_data):
        """
        Populate a single row with persistent volume data using efficient methods
        
        Args:
            row: The row index
            pv_data: List containing persistent volume information
        """
        # Set row height once
        ...
        for col, value in enumerate(pv_data):
    ...
    def _handle_action(self, action, row):
        """Override base action handler for persistent volume-specific actions"""
        pv_name = self.table.item(row, 1).text()
        if action == "Edit":
            print(f"Editing persistent volume: {pv_name}")
        elif action == "Delete":
            print(f"Deleting persistent volume: {pv_name}")
    ...
    def handle_row_click(self, row, column):
        ...
        if column != self.table.columnCount() - 1:  # Skip action column
            ...
            # Log selection
            pv_name = self.table.item(row, 1).text()
            print(f"Selected persistent volume: {pv_name}")
