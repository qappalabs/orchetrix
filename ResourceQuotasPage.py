...
class ResourceQuotasPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.selected_resource_quotas = set()  # Track selected resource quotas
        self.setup_ui()
        self.load_data()
    ...
    def handle_row_click(self, row, column):
        if column != 4:  # Don't trigger for action button column
            resource_quota_name = self.table.item(row, 1).text()
            print(f"Clicked resource quota: {resource_quota_name}")
            # Add your resource quota click handling logic here
    ...
    def handle_action(self, action, row):
        resource_quota_name = self.table.item(row, 1).text()
        if action == "View":
            print(f"Viewing resource quota: {resource_quota_name}")
            # Add view logic here
        elif action == "Edit":
            print(f"Editing resource quota: {resource_quota_name}")
            # Add edit logic here
        elif action == "Delete":
            print(f"Deleting resource quota: {resource_quota_name}")
            # Add delete logic here

    def create_checkbox(self, row, resource_quota_name):
        checkbox = QCheckBox()
        ...
        checkbox.stateChanged.connect(lambda state: self.handle_checkbox_change(state, resource_quota_name))
        return checkbox

    def create_checkbox_container(self, row, resource_quota_name):
        ...
        checkbox = self.create_checkbox(row, resource_quota_name)
        layout.addWidget(checkbox)

        return container

    def handle_checkbox_change(self, state, resource_quota_name):
        if state == Qt.CheckState.Checked.value:
            self.selected_resource_quotas.add(resource_quota_name)
        else:
            self.selected_resource_quotas.discard(resource_quota_name)
        print(f"Selected resource quotas: {self.selected_resource_quotas}")
    ...
    def load_data(self):
        # Sample data for resource quotas
        resource_quotas_data = [
            ["default-quota", "default", "71d"],
            ["dev-quota", "dev", "45d"],
            ["system-quota", "kube-system", "71d"],
            ["test-quota", "test", "30d"],
            ["production-quota", "production", "71d"]
        ]

        self.table.setRowCount(len(resource_quotas_data))

        for row, quota in enumerate(resource_quotas_data):
            # Set row height
            self.table.setRowHeight(row, 40)

            # Add checkbox in a container to prevent selection issues
            checkbox_container = self.create_checkbox_container(row, quota[0])
            self.table.setCellWidget(row, 0, checkbox_container)

            # Name column
            item_name = QTableWidgetItem(quota[0])
            ...
            # Namespace column
            item_namespace = QTableWidgetItem(quota[1])
            ...
            # Age column
            days = int(quota[2].replace('d', ''))
            item_age = SortableTableWidgetItem(quota[2], days)
        ...
        self.items_count.setText(f"{len(resource_quotas_data)} items")
