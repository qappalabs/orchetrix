...
class BaseTablePage(QWidget):
    ...
    def _configure_table_resizing(self, table, headers):
        """Configure table columns for proper resizing and full width utilization"""
        header = table.horizontalHeader()
        
        # Essential header configuration for proper resizing
        ...
        # Configure resize modes for each column
        ...
        # Set the second-to-last column to stretch to fill remaining space
        if len(headers) > 2:
            stretch_column = len(headers) - 2  # Second to last column (before actions)
