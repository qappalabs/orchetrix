...
class KubernetesMetricsService:
    ...
    def _calculate_cluster_metrics(self) -> Dict[str, Any]:
        try:
            ...
            cpu_usage_percent = (
                (cpu_requests_total / cpu_total_cores * 100)
                if cpu_total_cores > 0
                else 0
            )
            memory_usage_percent = (
                (memory_requests_total / memory_total_bytes * 100)
                if memory_total_bytes > 0
                else 0
            )
            pods_usage_percent = (
                (running_pods_count / pods_capacity_total * 100)
                if pods_capacity_total > 0
                else 0
            )
    ...
    def get_all_node_metrics_fast(
        self, node_names: list = None, include_disk_usage: bool = False
        ...
        try:
            ...
            if len(all_nodes_items) > MAX_NODES_PER_BATCH:
                ...
                return self._process_nodes_in_batches(
                    all_nodes_items, include_disk_usage
                )
            ...
            while True:
                ...
                logging.debug(
                    f"Fetched {batch_size} pods in batch (total: {total_pods_fetched})"
                )
                continue_token = getattr(response.metadata, "_continue", None)
                if not continue_token:
                    break
            ...
            for i, node in enumerate(nodes_list.items):
                ...
                try:
                    ...
                    metrics = self._calculate_single_node_metrics_fast(
                        node, pods_by_node.get(node_name, []), include_disk_usage
                    )
                    if metrics:
    ...
    def _process_nodes_in_batches(
        self, all_nodes: list, include_disk_usage: bool
    ) -> Dict[str, Dict[str, Any]]:
        try:
            ...
            for i in range(0, len(all_nodes), BATCH_SIZE):
                ...
                # Get pods per node using server-side filtering
                pods_by_node = {}
                for node in batch_nodes:
                    node_name = node.metadata.name
                    try:
                        node_pods = self.api_service.v1.list_pod_for_all_namespaces(
                        ...
                        if node_pods.items:
                            pods_by_node[node_name] = node_pods.items
                    except Exception as e:
                        logging.warning(
                            f"Failed to get pods for node {node_name}: {e}"
                        )
    ...
    def _calculate_single_node_metrics_fast(
        self, node, node_pods: list, include_disk_usage: bool = False
        ...
        try:
            ...
            if real_usage:
                ...
                storage_requests = 0
                cpu_usage_percent = (
                    (real_usage.get("cpu_usage", 0) / cpu_capacity * 100)
                    if cpu_capacity > 0
                    else 0
                )
                memory_usage_percent = (
                    (real_usage.get("memory_usage", 0) / memory_capacity * 100)
                    if memory_capacity > 0
                    else 0
                )
                disk_usage_percent = (
                    real_usage.get("disk_usage_percent", 0)
                ...
                memory_usage_percent = (
                    (memory_requests / memory_capacity * 100)
                    if memory_capacity > 0
                    else 0
                )
                ...
                if include_disk_usage:
                    ...
                    if disk_usage_percent == 0.0:
                        ...
                        disk_usage_percent = (
                            (storage_requests / storage_capacity * 100)
    ...
    def get_node_metrics(self, node_name: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            real_usage = self._get_real_node_usage_from_metrics_server(
                node_name, node, None
            )  # Pods not pre-fetched here, so pass None
            if real_usage:
                # Use real metrics if available
                cpu_requests = 0
                memory_requests = 0
    ...
    def _get_real_node_usage_from_metrics_server(
        self, node_name: str, node=None, pods: list = None
    ) -> Optional[Dict[str, Any]]:
        try:
            ...
            try:
                ...
                if node is not None and pods is not None:
                    ...
                    disk_usage_percent = self._calculate_real_disk_usage_legacy(
                        node_name
                    )
                logging.debug(
                    f"Retrieved real metrics from metrics - server for {node_name}: "
                ...
                logging.debug(
                    f"Error getting metrics from metrics server for {node_name}: {e}"
                )
                return None
        except Exception as e:
