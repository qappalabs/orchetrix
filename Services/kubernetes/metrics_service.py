...
class KubernetesMetricsService:
        ...
        try:
            ...
            for node in nodes_list.items:
                if node.status and node.status.capacity:
                    ...
                    if node.status.allocatable:
                        cpu_allocatable = self._parse_cpu_value(
                    ...
                    memory_capacity = self._parse_memory_value(
                        node.status.capacity.get('memory', '0Ki'))
                    memory_total_bytes += memory_capacity

                    if node.status.allocatable:
                        memory_allocatable = self._parse_memory_value(
                            node.status.allocatable.get('memory', '0Ki'))
                        memory_allocatable_bytes += memory_allocatable
                    else:
                        memory_allocatable_bytes += memory_capacity

                    # Pod capacity
                    pods_capacity_total += int(
                        node.status.capacity.get('pods', '110'))
                            ...
                            if container.resources:
                                ...
                                if container.resources.requests:
                                    cpu_request = container.resources.requests.get(
                                        'cpu', '0')
                                    cpu_requests_total += self._parse_cpu_value(
                                        cpu_request)

                                    memory_request = container.resources.requests.get(
                                        'memory', '0')
                                    memory_requests_total += self._parse_memory_value(
                                        memory_request)

                                # CPU limits
                                if container.resources.limits:
                                    cpu_limit = container.resources.limits.get(
                                        'cpu', '0')
                                    cpu_limits_total += self._parse_cpu_value(
                                        cpu_limit)

                                    memory_limit = container.resources.limits.get(
                                        'memory', '0')
                                    memory_limits_total += self._parse_memory_value(
                                        memory_limit)

            # Calculate usage percentages based on requests vs capacity
            cpu_usage_percent = (
                cpu_requests_total / cpu_total_cores * 100) if cpu_total_cores > 0 else 0
            memory_usage_percent = (
                memory_requests_total / memory_total_bytes * 100) if memory_total_bytes > 0 else 0
            pods_usage_percent = (
                running_pods_count / pods_capacity_total * 100) if pods_capacity_total > 0 else 0
            ...
            metrics = {
                ...
                "memory": {
                    ...
                    # Convert to MB
                    "requests": round(memory_requests_total / (1024**2), 2),
                    # Convert to MB
                    "limits": round(memory_limits_total / (1024**2), 2),
                    # Convert to MB
                    "allocatable": round(memory_allocatable_bytes / (1024**2), 2),
                    # Convert to MB
                    "capacity": round(memory_total_bytes / (1024**2), 2)
                },
                "pods": {
    ...
    def get_all_node_metrics_fast(self, node_names: list = None, include_disk_usage: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            # Get all nodes using pagination to handle large clusters
            all_nodes_items = []
            continue_token = None

            while True:
                response = self.api_service.v1.list_node(
                    limit=MAX_NODES_PER_BATCH,
                    _continue=continue_token,
                    _request_timeout=APIClientConfig.METRICS_COLLECTION_TIMEOUT
                )

                all_nodes_items.extend(response.items)
            ...
            if node_names:
                all_nodes_items = [
                    node for node in all_nodes_items if node.metadata.name in node_names]

            # Process in batches if too many nodes
            if len(all_nodes_items) > MAX_NODES_PER_BATCH:
                logging.info(
                    f"Large cluster detected ({len(all_nodes_items)} nodes), processing in batches")
                return self._process_nodes_in_batches(all_nodes_items, include_disk_usage)

            # Create a mock object to hold items for compatibility with rest of the code
            class NodesList:
                def __init__(self, items):
                    self.items = items

            nodes_list = NodesList(all_nodes_items)

            # Get ALL pods for all namespaces using pagination to handle large clusters
            all_pods_items = []
            continue_token = None
            total_pods_fetched = 0
            ...
            while True:
                ...
                all_pods_items.extend(response.items)
                total_pods_fetched += batch_size

                logging.debug(f"Fetched {batch_size} pods in batch (total: {total_pods_fetched})")
            ...
            for i, node in enumerate(nodes_list.items):
                ...
                try:
                    ...
                    if i % 10 == 0 and len(nodes_list.items) > 20:
                        logging.debug(
                            f"Processing node {i + 1}/{len(nodes_list.items)}: {node_name}")
                    ...
                    logging.warning(
                        f"Error calculating fast metrics for node {node_name}: {e}")
                    failed_nodes.append(node_name)
                    # Set default metrics for failed nodes
                    all_metrics[node_name] = self._get_default_node_metrics(
                        node_name)
    ...
    def _process_nodes_in_batches(self, all_nodes: list, include_disk_usage: bool) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(0, len(all_nodes), BATCH_SIZE):
                ...
                logging.info(
                    f"Processing batch {i // BATCH_SIZE + 1}/{(len(all_nodes) + BATCH_SIZE - 1) // BATCH_SIZE}: {len(batch_nodes)} nodes")

                # Get pods for this batch of nodes only
                node_names_in_batch = [
                    node.metadata.name for node in batch_nodes]
                ...
                batch_pods = self.api_service.v1.list_pod_for_all_namespaces(
                    ...
                    _request_timeout=APIClientConfig.HEAVY_LOAD_TIMEOUT
                )
                ...
                for node in batch_nodes:
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Error calculating metrics for node {node_name} in batch: {e}")
                        all_metrics[node_name] = self._get_default_node_metrics(
                            node_name)

                batch_time = (time.time() - batch_start_time) * 1000
                logging.info(
                    f"Batch {i // BATCH_SIZE + 1} completed in {batch_time:.1f}ms")
    ...
    def _calculate_single_node_metrics_fast(self, node, node_pods: list, include_disk_usage: bool = False) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if node.status.allocatable:
                ...
                memory_allocatable = self._parse_memory_value(
                    node.status.allocatable.get('memory', '0Ki'))
                storage_allocatable = self._parse_storage_value(
                    node.status.allocatable.get('ephemeral-storage', '0Ki'))

            # Try to get real usage from metrics server first for more accurate data
            real_usage = self._get_real_node_usage_from_metrics_server(
                node_name, node, node_pods)
            ...
            if real_usage:
                ...
                cpu_usage_percent = (real_usage.get(
                    'cpu_usage', 0) / cpu_capacity * 100) if cpu_capacity > 0 else 0
                memory_usage_percent = (real_usage.get(
                    'memory_usage', 0) / memory_capacity * 100) if memory_capacity > 0 else 0
                disk_usage_percent = real_usage.get(
                    'disk_usage_percent', 0) if include_disk_usage else None

                # Still count running pods from pre - fetched data
                running_pods = sum(
                    1 for pod in node_pods if pod.status and pod.status.phase == "Running")
                ...
                memory_usage_percent = (
                    memory_requests / memory_capacity * 100) if memory_capacity > 0 else 0

                # Get disk usage if requested
                if include_disk_usage:
                    disk_usage_percent = self._calculate_real_disk_usage(
                        node, node_pods)
                    if disk_usage_percent == 0.0:
                        # Fallback to storage requests calculation only if no real data
                        disk_usage_percent = (
                            storage_requests / storage_capacity * 100) if storage_capacity > 0 else 0.0
                else:
                    disk_usage_percent = None

            # Calculate pods usage percentage (same for both real and request - based metrics)
            pods_usage_percent = (
                running_pods / pods_capacity * 100) if pods_capacity > 0 else 0
            ...
            logging.error(
                f"Error calculating fast metrics for node {node.metadata.name}: {e}")
    ...
    def get_node_metrics(self, node_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            real_usage = self._get_real_node_usage_from_metrics_server(
                node_name, node, None)  # Pods not pre-fetched here, so pass None
            ...
            if real_usage:
                ...
                running_pods = sum(
                    1 for pod in pods_list.items if pod.status and pod.status.phase == "Running")

                # For real metrics, we don't have request values, so keep them as 0
                logging.debug(
                    f"Using real metrics for {node_name}: CPU {cpu_usage_percent:.1f}%, Memory {memory_usage_percent:.1f}%")
            else:
                # Fallback to resource requests calculation
                logging.debug(
                    f"No metrics server data for {node_name}, using resource requests as fallback")
                ...
                disk_usage_percent = self._calculate_real_disk_usage_legacy(node_name)
            ...
            if disk_usage_percent == 0.0:
                if storage_capacity > 0 and storage_requests > 0:
                    ...
                    disk_usage_percent = min(
                        (storage_requests / storage_capacity * 100), 100.0)
                else:
                    # No data available
                    disk_usage_percent = 0.0

                logging.debug(
                    f"Using storage requests for disk usage {node_name}: {disk_usage_percent:.1f}%")
            ...
            logging.error(
                f"API error getting node metrics for {node_name}: {e}")
            return None
        except Exception as e:
            logging.error(f"Error getting node metrics for {node_name}: {e}")
            return None

    def _get_real_node_usage_from_metrics_server(self, node_name: str, node=None, pods: list = None) -> Optional[Dict[str, Any]]:

        try:
            if not hasattr(self.api_service, 'custom_objects_api'):
                logging.warning(
                    "No custom objects API available for metrics server")
                return None
            ...
            try:
                ...
                if not metrics or 'usage' not in metrics:
                    logging.debug(
                        f"No usage data in metrics response for {node_name}")
                    return None
                ...
                disk_usage_percent = 0.0
                if node is not None and pods is not None:
                    disk_usage_percent = self._calculate_real_disk_usage(node, pods)
                else:
                    # Fallback to old behavior if parameters not provided
                    disk_usage_percent = self._calculate_real_disk_usage_legacy(node_name)
                ...
                logging.debug(f"Error getting metrics from metrics server for {node_name}: {e}")
                return None

        except Exception as e:
            logging.debug(
                f"Error accessing metrics server for {node_name}: {e}")
            return None

    def _calculate_real_disk_usage_legacy(self, node_name: str) -> float:
        """Legacy method for backward compatibility when node and pods are not pre-fetched"""
        try:
            # Get node capacity
            node = self.api_service.v1.read_node(name=node_name)
            if not node.status or not node.status.capacity:
                            ...
                            if volume.persistent_volume_claim:
                                ...
                                try:
                                    ...
                                    if pvc.spec and pvc.spec.resources and pvc.spec.resources.requests:
                                        ...
                                        pvc_estimated_usage = self._parse_storage_value(
                                            pvc_size_request) * 0.6
            ...
            if running_pods_count > 0:
                ...
                image_overhead = running_pods_count * 50 * \
                    1024 * 1024  # 50MB per pod for overlays
