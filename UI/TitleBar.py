...
#         """Handle double-click events for maximizing/restoring the window"""
...
#             # If maximized, restore before allowing drag
#             if self.parent.isMaximized():
#                 self.parent.showNormal()
#                 # Update the maximize button icon
#                 self.update_maximize_button_icon(False)

#                 # After restoring, we need to adjust drag position
#                 # Set it to the position relative to the top-left of the window
#                 # This ensures smooth drag operation after restoring
#                 cursor_pos = event.globalPosition().toPoint()
#                 window_pos = self.parent.pos()
#                 local_pos = event.position().toPoint()

#                 # Calculate where the window should be positioned
#                 # to make the drag point (cursor) be at the same relative position
#                 new_window_pos = cursor_pos - local_pos
#                 self.parent.move(new_window_pos)

#                 # Update drag position for subsequent mouseMoveEvent
#                 self.drag_position = cursor_pos
#             else:
#                 # If not maximized, just maximize
#                 self.parent.showMaximized()
#                 # Update the maximize button icon
#                 self.update_maximize_button_icon(True)
#                 # Clear drag position to prevent unwanted drags when maximized
...
#         # Only drag if we have a valid drag position and the window is not maximized
#         if (self.drag_position is not None and
#                 event.buttons() == Qt.MouseButton.LeftButton and
#                 self.parent and
#                 not self.parent.isMaximized()):

#             # Calculate the delta and move the window
#             delta = event.globalPosition().toPoint() - self.drag_position
#             self.parent.move(self.parent.pos() + delta)
#             self.drag_position = event.globalPosition().toPoint()

#     def mouseReleaseEvent(self, event):
#         """Reset the drag position and double-click flag on mouse release"""
#         if event.button() == Qt.MouseButton.LeftButton:
#             self.drag_position = None
